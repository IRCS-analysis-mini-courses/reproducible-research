...
<<load experimental data, echo = FALSE, eval = TRUE, message = FALSE>>=
setwd('~/GitHub/reproducible-research/Day-3/sweave_knitr/m100-sweave/')
...
\begin{center}
<<case significance Tukey tests, echo = FALSE, message = FALSE, results = tex>>=
library(ggplot2)
kable(fortify(child.addmodel.case.tukey), format = 'latex', digits = 1)
...
\begin{center}
<< warning = FALSE, echo = FALSE, message = FALSE, fig=TRUE>>=
library(grid)
